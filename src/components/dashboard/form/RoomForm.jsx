...
export default function RoomForm({ room }) {
	...
	const submitHandler = e => {
		...
		if (room?.id) {
			axios.put('/api/cabanas', {form, id: room.id})
				.then(resp => {
								...
								{types.map((type, i) => (
									<label 
										...
										<span 
											...
											{ form.type == type ? <span className="bg-primary w-2.5 h-2.5 block rounded-full" /> : null}
										</span>
									...
									{services.map((service, i) => (
										<div key={`service-${i}`}>
											<label 
												...
												<span className="text-black text-sm leading-none pt-1 font-medium">{service}</span>
											</label>
										</div>
									))}
								</div>
